exports.command = {
	...
	execute: function(socket, command, command_access) {
		...
		if ((typeof command === 'undefined') || command.length < 1) {
			command_access.sendData(socket, chalk.yellow("[::] ") + "You have to use it this way: .desc <description here>\r\n");
		} else if (command.length > descMaxLength) {
			command_access.sendData(socket, chalk.yellow("[::] ") + "Your description can't have more than " + chalk.bold(descMaxLength) + " characters.\r\n");
		} else {
			socket.db.desc = command;
			command_access.updateUser(socket.username, socket.db);
			command_access.sendData(socket, chalk.green("[::] ") + " Your description is now: " + command + "\r\n");
		}
	}
}
