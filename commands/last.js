	...
	execute: function(socket, command, command_access) {
		...
		if (typeof whom === 'undefined' || whom.length < 1) {
					...
					for (var j = 0; j < cleanlist.length; j++) {
						if (cleanlist[j].loginTime === sliced[i]) {
							command_access.sendData(socket, "  " + chalk.yellow(cleanlist[j].username) +
								" logged in at " + chalk.bold(command_access.getDateTimeString(cleanlist[j].loginTime)) + ".  ");
			...
			if (typeof w.loginTime !== 'undefined') {
				command_access.sendData(socket, chalk.green(":: " + chalk.cyan(whom) + " last logged in at " + chalk.bold(command_access.getDateTimeString(w.loginTime)) + ".\r\n"));
			} else {
